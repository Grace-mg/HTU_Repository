...
export default function AcceptInvitePage() {
  ...
  const [detectedEmail, setDetectedEmail] = React.useState(emailParam);
  ...
  React.useEffect(() => {
    // Check if Supabase browser client has an active invite session from URL token
    const client = createBrowserClient();
    client.auth.getUser().then(({ data }) => {
      if (data?.user) {
        if (data.user.email && !detectedEmail) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await authService.acceptAdminInvite(password, fullName, detectedEmail);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-background px-4 py-12">
      <div className="w-full max-w-md space-y-6 bg-card p-8 rounded-xl border border-border shadow-lg">
        {/* Header section */}
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <Input
                type="email"
                readOnly
                value={detectedEmail || "admin.candidate@htu.edu.gh"}
